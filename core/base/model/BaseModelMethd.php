...
abstract class BaseModelMethod 
{
    ...
    protected function createWhere($set, $table = false, $instruction = 'WHERE') {
        ...
        if(is_array($set['where']) && !empty($set['where'])) {
            ...
            $set['operand'] = (is_array($set['operand']) && !empty($set['opernad'])) ? $set['operand'] : ['=']; 
            ...
            foreach($set['where'] as $key => $item) {
                ...
                if($set['operand'][$o_count]) {
                    ...
                    $opernad = $set['operand'][$o_count - 1];
                }

                if($set['condition'][$o_count]) {
                    $condition = $set['condition'][$o_count];
                    $c_count++;
                } else {
            ...
            $where = substr($where, 0, strpos($where, $condition));
                ...
                if($item['on']) {
                    ...
                    if($item['on']['table']) $join .= $item['on']['table'];
                        ...
                        $group_condition = 'WHERE';
                            ...
                            $group_condition = $item['groupe_condition'] ? strtoupper($item['groupe_condition']) : 'AND';
                        }
        ...
        foreach($fields as $i => $item) {
            ...
            if($j < $count_fields) {
                for(; $j < $count_fields; $j++) {
                    $insert_arr['values'] .= "'" . addslashes($value) . "NULL" . ',';
                }
            }
